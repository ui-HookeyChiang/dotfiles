...
3. 架構決定?           → ADR (docs/adr/)
4. 已做完?             → git / PR
5. bug / 待辦 / 延後?   → triage 建單 → docs/ticket/
6. n=1 觀察待評估?      → triage 建單 → docs/ticket/ Status: needs-triage
                         (再犯加進同單 → triage 見 n≥2 → 升護欄)
7. 不可機制化跨-session 環境事實? → MEMORY (一行 ≤200 字)  ← 唯一進 MEMORY
```

Paths (`docs/ticket/`, `docs/adr/`) are the skill-dev backend; other repos resolve
