...
CLI mirrors the SKILL.md argument-hint:
    <path-to-SKILL.md> [--cross <dir>] [--axis G1|G8|IRD|GC|all]
                       [--no-llm]
...
its metrics-only path and G8's dispatch callable is also ``None``.  The env var
...
def _normalise_axis(value: str) -> str:
    """argparse type=: uppercase named axes but lowercase 'all' (axis aliases).
...
def _render_finding_yaml(finding: dict, base_indent: int = 4) -> str:
    ...
    nested mapping when a detector provides one and as ``null`` otherwise.
    """
    pad = " " * base_indent
    out: list[str] = []
...
def _resolve_llm_dispatch() -> Optional[Callable[..., Any]]:
    ...
    if not module_path or not attr:
        print(
            f"semantic_audit.py: ignoring {_ENV_LLM_DISPATCH}={spec_str!r} — "
    ...
    try:
        ...
        print(
            f"semantic_audit.py: failed to resolve {_ENV_LLM_DISPATCH}={spec_str!r} "
    ...
    if not callable(fn):
        print(
            f"semantic_audit.py: {_ENV_LLM_DISPATCH}={spec_str!r} resolved to a "
...
def _dispatch(
    ...
    """Call the named detector with a shared semantic detector interface."""
    if axis == "G1":
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not os.path.exists(args.skill_md):
        print(f"semantic_audit.py: input not found: {args.skill_md}", file=sys.stderr)
        return EXIT_FAILURE
    ...
    for axis in _axes_to_run(args.axis):
        try:
            ...
            print(
                "semantic_audit.py: G1 found fewer than 2 candidate paragraphs "
            ...
            print(f"semantic_audit.py: {exc}", file=sys.stderr)
            return EXIT_FAILURE
            ...
            print(
                f"semantic_audit.py: {axis} detector not implemented ({exc}); "
            ...
            print(
                f"semantic_audit.py: {axis} detector raised RuntimeError ({exc}); "
        ...
        if not isinstance(findings, list):
            print(
                f"semantic_audit.py: {axis} detector returned non-list "
    ...
    for na in na_advisories:
        print(
            f"semantic_audit.py: NOT_APPLICABLE [{na.get('axis', '?')}] "
