...
echo '{"voter":"A2","verdict":"NOT_APPLICABLE","confidence":"high","concerns":[]}' > "$TMP/private-A2/ballot.json"
...
echo '{"voter":"A5","verdict":"NOT_APPLICABLE","confidence":"high","concerns":[]}' > "$TMP/private-A5/ballot.json"
python3 "$HERE/voting-harness.py" aggregate "$TMP" | python3 -c "
import json, sys
r = json.loads(sys.stdin.read())
assert r['outcome'] == 'NEEDS_HUMAN', f'got {r[\"outcome\"]}'
assert r['voting_total'] == 2
assert 'corpus too thin' in r['count_note']
print('PASS: aggregate-voting-total-lt3')
"
