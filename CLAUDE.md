...
- `skill-dev/` — private submodule (github.com/ui-HookeyChiang/skill-dev): canonical source for skills (repo-root skill dirs), `hooks/`, `scripts/`, `_shared/`, and `home/` (user-global config).
- `skill-dev/home/CLAUDE.md` — GLOBAL instructions (symlinked to `~/.claude/CLAUDE.md` by the installer). Edit in the submodule, never in `~/.claude/` directly.
- `skill-dev/home/docs/agents/` — docs `@imported` by the global CLAUDE.md.
- `.claude` — compat symlink to `skill-dev/home` (keeps installer whitelist paths stable). Not a directory; do not add files under it.
