...
r="$(wt_name solo)"; rc=$?
chk "t5 wt_name top-level" "solo" "$r" 0 $rc
r="$(wt_name "my wt/task 2")"; rc=$?
chk "t6 wt_name space" "my wt" "$r" 0 $rc
r="$(wt_task ns/task-1)"; rc=$?
chk "t7 wt_task simple" "task-1" "$r" 0 $rc
r="$(wt_task foo)"; rc=$?
...
chk "t11 wt_id nested" "ns:task-1" "$r" 0 $rc
r="$(wt_id /repo /repo/.worktrees/solo)"; rc=$?
chk "t12 wt_id top-level" "solo" "$r" 0 $rc
